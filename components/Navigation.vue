	...
	<nav class="sticky top-0 bg-theme-color-300 z-40 shadow-md">
		<div class="cont grid grid-cols-[auto_1fr_auto] p-2 gap-4 items-center">
			...
			<div class="flex gap-2">
				...
				<NuxtLink href="/login" class="bg-theme-color-500 px-2 py-0.5 flex items-center rounded"> <IconLogin :class="route.meta.user ? 'rotate-180' : ''" /> {{ route.meta.user ? 'Logout' : 'Login' }} </NuxtLink>
			</div>
		</div>
	</nav>
</template>
